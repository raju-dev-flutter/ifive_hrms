...
class ProjectTaskUpdateFormScreen extends StatefulWidget {
  final TaskData task;

  const ProjectTaskUpdateFormScreen({super.key, required this.task});

  @override
  State<ProjectTaskUpdateFormScreen> createState() =>
      _ProjectTaskUpdateFormScreenState();
}

class _ProjectTaskUpdateFormScreenState
    extends State<ProjectTaskUpdateFormScreen> with InputValidationMixin {
  final GlobalKey<ScaffoldState> _scaffoldKey = GlobalKey<ScaffoldState>();
  final GlobalKey<FormState> _formKey = GlobalKey<FormState>();

  final taskUpdateStream = sl<ProjectTaskUpdateStream>();

  @override
  void initState() {
    super.initState();
    taskUpdateStream.fetchInitialCallBack(widget.task);
  }
  ...
  void blocListener(BuildContext context, ProjectTaskCrudState state) {
    if (state is ProjectTaskCrudSuccess) {
      ...
      AppAlerts.displaySnackBar(context, "Task Successfully Updated", true);
    }
          ...
          return Form(
            ...
            child: Column(
              ...
              children: [
                  ...
                  containerCard(
                    child: Column(
                      ...
                      children: [
                        ...
                        for (var i = 0;
                            i < widget.task.taskLogData!.length;
                            i++) ...[
                          containerCard(
                ...
                Dimensions.kVerticalSpaceSmall,
                if (widget.task.status!.toUpperCase() == "INITIATED" ||
                    widget.task.status!.toUpperCase() == "PENDING")
                  containerCard(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        ...
                        //               color: appColor.bgQuaternary,
                        //               textColor: appColor.textPlaceholder,
                        //               label: "REJECTED",
                        //             )
                        //           : ActionButton(
                        //               onPressed: () => changeState(""),
                        //               color: appColor.bgErrorSolid,
                        //               textColor: appColor.textPrimaryOnBrand,
                        //               label: "REJECTED",
                        //             ),
                        //     ),
                        //     Dimensions.kHorizontalSpaceSmaller,
                        ...
                        //           ? ActionButton(
                        //               onPressed: () => changeState("COMPLETED"),
                        //               color: appColor.bgQuaternary,
                        //               textColor: appColor.textPlaceholder,
                        //               label: "COMPLETED",
                        //             )
                        ...
                        StreamBuilder<List<File>>(
                            stream: taskUpdateStream.captureImageSubject,
                        ...
                        CustomTextFormField(
                          label: "Completed Percentage",
                          controller:
                              taskUpdateStream.completedPercentageController,
                          maxLines: 1,
                          ...
                          onChanged: (val) {
                            if (int.parse(val!) > 100) {
                              taskUpdateStream
                                  .completedPercentageController.text = "0";
                              AppAlerts.displaySnackBar(context,
                                  "Please enter the value below 100%", false);
                            }
                          },
                          required: true,
                        ),
                        ...
                        CustomTextFormField(
                          label: "Description",
                          controller: taskUpdateStream.descriptionController,
                          maxLines: 3,
                          required: true,
                          validator: (val) {
  ...
  void onCompletedSubmit() {
    if (_formKey.currentState!.validate()) {
      // if (taskUpdateStream.captureImageSubject.valueOrNull!.isNotEmpty) {
      taskUpdateStream.onSubmit(context, widget.task, status);
      // }
      // else {
      //   AppAlerts.displaySnackBar(
      //       context, "Please upload attachment....", false);
      // }
    }
  }
        ...
        Positioned(
          ...
          child: GestureDetector(
            onTap: () {
              ...
              taskUpdateStream.addCaptureImage(listDocument);
            },
  ...
  void selectCameraPicker() async {
    ...
    List<File> oldDocument =
        taskUpdateStream.captureImageSubject.valueOrNull ?? [];
    final pickedFile = await imagePicker.pickImage(source: ImageSource.camera);

    if (pickedFile != null) {
      oldDocument.add(File(pickedFile.path));
      taskUpdateStream.addCaptureImage(oldDocument);
      setState(() {});
    }
  }
