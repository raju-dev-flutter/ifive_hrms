...
class PipelineDatabaseUpdateStream {
  PipelineDatabaseUpdateStream(
  ...
  /// Pipeline Details
  late TextEditingController scopeController = TextEditingController();
  late TextEditingController priceInLakhsController = TextEditingController();
  ...
  final _winningProbList = BehaviorSubject<List<CommonList>>.seeded([]);
  final _winningProbListInit = BehaviorSubject<CommonList>();

  final _stageList = BehaviorSubject<List<CommonList>>.seeded([]);
  final _stageListInit = BehaviorSubject<CommonList>();
  ...
  Stream<List<CommonList>> get winningProbList => _winningProbList;

  Stream<List<CommonList>> get stageList => _stageList;

  Stream<List<CommonList>> get statusList => _statusList;
  ...
  ValueStream<CommonList> get winningProbListInit => _winningProbListInit;

  ValueStream<CommonList> get stageListInit => _stageListInit;

  ValueStream<CommonList> get statusListInit => _statusListInit;
  ...
  void fetchInitialCallBack(DatabaseData database) async {
    ...
    response.fold(
      (_) => null,
      (_) {
        if (_.winningProb!.isNotEmpty) {
          _winningProbList.sink.add(_.winningProb ?? []);
        }
        if (_.stage!.isNotEmpty) {
          _stageList.sink.add(_.callResponse ?? []);
        }
        if (_.status!.isNotEmpty) {
          _statusList.sink.add(_.status ?? []);
        }
  ...
  void winningProb(params) {
    _winningProbListInit.sink
        .add(CommonList(id: params.value, name: params.name));
  }
  ...
  void stage(params) {
    _stageListInit.sink.add(CommonList(id: params.value, name: params.name));
  }
  ...
  void onSubmit(BuildContext context, DatabaseData database) {
    final pipelineDetails = {
      "lead_id": database.leadId,
      "scope": scopeController.text,
      "bussiness_volume": priceInLakhsController.text,
      "winning_prob": _winningProbListInit.valueOrNull!.name ?? "",
      "stage": _stageListInit.valueOrNull!.id ?? "",
      ...
      "winning_prob_id": _winningProbListInit.valueOrNull!.id ?? "",
      "status_id": _statusListInit.valueOrNull!.id ?? "",
    ...
    BlocProvider.of<SfaCrudBloc>(context)
        .add(GenerateTicketEvent(body: body, type: 'Pipeline'));
  }

  void onClear() {
    /// Pipeline Details
    scopeController.clear();
    priceInLakhsController.clear();
