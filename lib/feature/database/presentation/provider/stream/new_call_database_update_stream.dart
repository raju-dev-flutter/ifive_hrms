...
class NewCallDatabaseUpdateStream {
  NewCallDatabaseUpdateStream(
  ...
  final _callStatusList = BehaviorSubject<List<CommonList>>.seeded([]);
  final _callStatusListInit = BehaviorSubject<CommonList>();

  final _callResponseList = BehaviorSubject<List<CommonList>>.seeded([]);
  final _callResponseListInit = BehaviorSubject<CommonList>();
  ...
  Stream<List<CommonList>> get callStatusList => _callStatusList;

  Stream<List<CommonList>> get callResponseList => _callResponseList;
  ...
  ValueStream<CommonList> get callStatusListInit => _callStatusListInit;

  ValueStream<CommonList> get callResponseListInit => _callResponseListInit;
  ...
  void fetchInitialCallBack(DatabaseData database) async {
    ...
    response.fold(
      (_) => null,
      (_) {
        if (_.callStatus!.isNotEmpty) {
          _callStatusList.sink.add(_.callStatus ?? []);
        }
        if (_.callResponse!.isNotEmpty) {
          _callResponseList.sink.add(_.callResponse ?? []);
        }
  ...
  void callStatus(params) {
    _callStatusListInit.sink
        .add(CommonList(id: params.value, name: params.name));
  }

  void callResponse(params) {
    _callResponseListInit.sink
        .add(CommonList(id: params.value, name: params.name));
  }
  ...
  void onSubmit(BuildContext context, DatabaseData database) {
    final newCallDetails = {
      "lead_id": database.leadId,
      "lead_source": leadSourceController.text,
      "call_status": _callStatusListInit.valueOrNull!.name ?? "",
      "call_response": _callResponseListInit.valueOrNull!.name ?? "",

      ///
      "call_response_id": _callResponseListInit.valueOrNull!.id ?? "",
      "call_status_id": _callStatusListInit.valueOrNull!.id ?? "",
