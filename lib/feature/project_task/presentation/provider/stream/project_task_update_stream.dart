...
class ProjectTaskUpdateStream {
  ProjectTaskUpdateStream({required TaskDeptLeadUseCase taskDeptLeadUseCase})
      : _taskDeptLeadUseCase = taskDeptLeadUseCase;

  final TaskDeptLeadUseCase _taskDeptLeadUseCase;

  final completedPercentageController = TextEditingController();
  ...
  void onSubmit(BuildContext context, TaskData task, status) {
    final DataMap body = {
      ...
      "completed_percentage":
          status == "REJECTED" ? "" : completedPercentageController.text,
      "description": descriptionController.text,
    };

    final List<File> captureFile =
        status == "REJECTED" ? [] : _captureImageSubject.valueOrNull ?? [];

    Logger().i("Submit: $body");

    BlocProvider.of<ProjectTaskCrudBloc>(context)
        .add(ProjectTaskUpdateEvent(body: body, file: captureFile));
  }
}
